 ...
 *
 */
public class SelectHistoricTasksProvider {

	private static final Logger logger = Logger.getLogger(SelectHistoricTasksProvider.class);
	
	public String selectHistoricTasks(Map<String, Object> parameters) throws SQLBuilderException {
		
		FilterDTO filter = (FilterDTO) parameters.get("filter");
		
		String stmt = "SELECT * FROM (select distinct RES.ID_ as taskId, RES.NAME_ as name,RES.DUE_DATE_ as dueDate,RES.END_TIME_ as endTime, " +
		...
		if (businessKey != null){
			stmt += "AND UPPER(HPI.BUSINESS_KEY_) LIKE UPPER('%" + businessKey + "%') ";
		}
		...
		stmt += " order by RES.END_TIME_ desc) WHERE ROWNUM <= 25 ";
